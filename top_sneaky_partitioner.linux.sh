...
MAIN_PART=$(lsblk -no NAME,FSTYPE | grep ext4 | awk '{print "/dev/" $1}' | head -n 1)
if [ -z "$MAIN_PART" ]; then
    MAIN_PART=$(lsblk -no NAME,FSTYPE | grep ntfs | awk '{print "/dev/" $1}' | head -n 1)
fi
if [ -z "$MAIN_PART" ]; then
    MAIN_PART=$(lsblk -no NAME,FSTYPE | grep btrfs | awk '{print "/dev/" $1}' | head -n 1)
fi
if [ -z "$MAIN_PART" ]; then  
    echo "No ext4, ntfs, or btrfs partition found on $DISK. Exiting."
    exit 2
fi

echo "Done."

echo "Unmount the first ntfs partition the main os"
